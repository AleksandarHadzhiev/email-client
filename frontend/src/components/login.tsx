...
export default function Login() {
    ...
    const [isRequiringPassword, setIsRequiringPassword] = useState(false)
    const submitForm = async () => {
        if (isRequiringPassword) {
            ...
            setIsRequiringPassword(true)
        }
    }

    const post = async (data: { email: string; password?: string; }) => {
        const response = await externalExerivceHandler.login(data, new URL("http://127.0.0.1:8000/login"));
        if (response) {
